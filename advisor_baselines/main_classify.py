...
def eval_net(args, net, dataloader, criterion):
    ...
    err, err_bad = 1.0*err_all/total, 1.0*err_bad_all/total 
    err, err_bad = str(round(err.item()*100,3))+'%', str(round(err_bad.item()*100,3))+'%'
    net.train()

    return 0, [err, err_bad], acc_list, predicteds.cpu().numpy()
...
def main(args, dataset):
    ...
    criterion = torch.nn.CrossEntropyLoss()  # nn.MSELoss()  #  nn.CrossEntropyLoss()  # defaul reduce is true
